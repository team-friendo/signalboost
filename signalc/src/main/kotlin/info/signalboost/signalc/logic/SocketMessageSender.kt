...
class SocketMessageSender(private val app: Application) {
    ...
    internal object WriterFactory {
        fun writerTo(socket: Socket): PrintWriter = PrintWriter(socket.getOutputStream(), true)
    }
    ...
    suspend fun stop(): Int = writerPool.clear()
        ...
        class WriterPool(coroutineScope: CoroutineScope) {
            internal val writers: MutableMap<SocketHashCode,Writer> = mutableMapOf()
                    ...
                    when (msg) {
                        is Message.Add -> {
                            ...
                            writers[socketHash]?.let {
                                ...
                                writers[socketHash] = Writer(coroutineScope, WriterFactory.writerTo(msg.socket))
        ...
        class Writer(private val coroutineScope: CoroutineScope, private val writer: PrintWriter) {
            sealed class Message {
                data class Send(val socketMsg: SocketOutMessage, val result: CompletableDeferred<Unit>) : Message()
