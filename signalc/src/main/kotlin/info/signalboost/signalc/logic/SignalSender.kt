...
class SignalSender(private val app: Application) {
    companion object {
        ...
        val paralellismFactor = (Application.availableProcessors * 2) + 1
    ...
    private fun messageQueueOf(account: VerifiedAccount): SendChannel<QueuedMessage> =
        getMemoized(messageQueues, account.username) {
            List(paralellismFactor) {
                Channel<QueuedMessage>(paralellismFactor).also { chan ->
...
/****************** FAILED THROTTLING ATTEMPTS ************************
 * these only ever send one at a time!
 */
...
//        getMemoized(messageQueues, account.username) {
//              ******* Actor way *******
////            app.coroutineScope.actor<QueuedMessage>(IO, MESSAGE_QUEUE_SIZE) {
////                for (qm in channel) {
////                    qm.result.complete(
////                        messageSenderOf(qm.sender).sendMessage(
////                            qm.recipient,
...
/*****************FAILED BATCHING ATTEMPTS
 *both not faster and get stuck when mesages are not increments of QUEUE_SIZE)
 */
//    private val messageQueues = ConcurrentHashMap<String,SendChannel<QueuedMessage>>()
