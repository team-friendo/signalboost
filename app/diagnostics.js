...
const {
  signal: { diagnosticsPhoneNumber, healtcheckInterval },
} = require('./config')
...
const sendHealthchecks = async () => {
  try {
    ...
    const responseTimes = await Promise.all(channelPhoneNumbers.map(signal.healthcheck))
...
// () => Promise<void>
const launchHealthcheckJob = () =>
  diagnosticsPhoneNumber
    ? util.repeatEvery(sendHealthchecks, healtcheckInterval)
    : Promise.resolve()
